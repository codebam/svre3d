import { Project, PhysicsLoader, THREE, ExtendedObject3D, ThirdPersonControls, PointerLock, PointerDrag, JoyStick } from 'enable3d';
...
export class MainScene extends CustomScene {
    ...
    wearAccessory(player, accessoryName) {
        ...
        if (player[item.accessory.type])
            head.remove(player[item.accessory.type]);
        head.add(item_mesh);
        player[item.accessory.type] = item_mesh;
    ...
    async create() {
        ...
        const box = this.physics.add.box({ x: 1, y: 2, z: -10, width: 5, height: 3, depth: 1, mass: 0, collisionFlags: 0 }, { lambert: { color: 'red', transparent: true, opacity: 0.5 } });
    ...
    update() {
        ...
        if (this.keys.w === true || this.player.move) {
            if (!this.player.isRunning && !this.player.isJumping)
                this.player.run({});
            const x = Math.sin(theta) * speed, y = this.player.player.body.velocity.y, z = Math.cos(theta) * speed;
