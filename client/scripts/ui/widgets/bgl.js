...
async function doBGL(canvas) {
    ...
    const player = await loadGlb('/resources/i/player/res');
    const TreeStructure = {
    ...
    function createBox(col) {
        const pos = generatePositions((col - 5) * 5, (elevation == 0 ? (rowIndex * 5) : player.scene.position.z) - 20);
        ...
        const box = new THREE.Mesh(new THREE.BoxGeometry(5, 1, 5));
        box.material = [
        ...
        const LogMaterial = new THREE.MeshToonMaterial({
            color: 0x660000,
            map: groundTextures.forest
        });
        if (structures[type] && pos.x !== 0 && pos.x !== 5) {
            const structure = structures[type];
            const rule = structure.structures[Random.pick(...Object.keys(structure.structures))];
            const name = Random.pick(...rule.name);
            let obj = structure.obj.clone();
            ...
            if (Random.from(0, parseInt(structure.density)) == 0)
                box.add(obj);
        }
        box.position.x = pos.x;
        box.position.y = elevation;
        box.position.z = pos.z;
        box.rotation.y = Random.pick(Math.PI / 2, Math.PI, 0);
    ...
    const colors = {
        sunrise: new THREE.Color(0xff4500),
        day: new THREE.Color(0xffffff),
        sunset: new THREE.Color(0xff4500),
        night: new THREE.Color(0x0000ff),
    ...
    const bloompass = new UnrealBloomPass(new THREE.Vector2(canvas.width, canvas.height), 0.5, 0.0, 1.01);
    ...
    // save pass
    const savePass = new SavePass(new THREE.WebGLRenderTarget(canvas.width, canvas.height, renderTargetParameters));
    // blend pass
    ...
    function spawnParticles(position) {
        ...
        for (let index = 0; index < count; index++) {
            ...
            const particlePosition = new THREE.Vector3(Random.from(fromPosition.x, toPosition.x), Random.from(fromPosition.y, toPosition.y), Random.from(fromPosition.z, toPosition.z));
    ...
    function updateParticles(deltaTime) {
        particles.forEach(particle => {
            ...
            if (data.life >= data.lifetime) {
                ...
                particle.material.opacity = alpha; // Fade out as lifetime progresses
            }
        });
    }
...
export function BGLogin() {
    ...
    return React.createElement("canvas", { className: "page-background", width: window.innerWidth, height: window.innerHeight, ref: canvasRef });
}
