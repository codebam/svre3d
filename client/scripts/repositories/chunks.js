...
export class Chunks {
    static renderCloseAndFar(playerPosition, renderDistance, detailsLimit, chunk) {
        if (Chunks._isCalculatingRenderDistance && !chunk)
            return;
        if (!chunk)
            Chunks._isCalculatingRenderDistance = true;
        ...
        const detailChunk = (chunk) => {
            ...
            const distance = Math.floor(new THREE.Vector3(playerPositionFloored.x, 0, playerPositionFloored.z).distanceTo(new THREE.Vector3(chunkPosition.x, 0, chunkPosition.z)));
        ...
        if (!chunk)
            Chunks._isCalculatingRenderDistance = false;
    }
    static setHighDetail(chunk) {
    ...
    static setMediumDetail(chunk) {
        chunk.object3d.traverse(object => {
            object.visible = true;
            let o = object;
            if (o.material) {
                if (!o.userData.__real__material__)
                    o.userData.__real__material__ = o.material;
    ...
    static setLowDetail(chunk) {
        chunk.object3d.traverse(object => {
            if (object.userData.info?.type !== 'chunk')
