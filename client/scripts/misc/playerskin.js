...
export class SkinPlayer {
    static createCanvasImage(player, colors, side, width = 1000, height = 1000) {
        ...
        for (let y = 0; y < height; y += pixelSize) {
            for (let x = 0; x < width; x += pixelSize) {
                const color = Random.pick(...colors, () => Math.abs(noise.perlin2(x * 0.01, y * 0.01) * 10)) || colors[0];
                grid[x + '-' + y] = color;
                ctx.fillStyle = color;
    ...
    static createNormalMap(grid, colors, width = 1000, height = 1000) {
        const canvas = document.createElement('canvas');
        const ctx = canvas.getContext('2d');
        console.log(grid);
        canvas.width = width;
        ...
        function getHeight(x, y) {
            ...
            return grid[closestPixel] ? (grid[closestPixel] == colors[0] ? 0x000000 : 0xffffff) : 0;
        }
        for (let y = 0; y < height; y += pixelSize) {
            for (let x = 0; x < width; x += pixelSize) {
