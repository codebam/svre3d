...
export class Entities {
    static spawn(type, position, name, variant, inventory, data, exp) {
        ...
        if (v) {
            ...
            if (v.trade)
                entity.data.trade = Array.isArray(v.trade) ? tradeOptions[Random.pick(...v.trade)] : tradeOptions[v.trade];
        }
        else {
    ...
    static startPing(socket, serverdata) {
        ...
        pingFrom(socket, 'entity:drop', ({ entity: eid, direction, item: itemID }) => {
            ...
            const pos = {
                ...
                y: entity.position.y,
                z: entity.position.z + direction.z,
            };
        ...
        pingFrom(socket, 'entities:getspawned', ({ id, position, distance }) => {
            let entities = this.entities.filter(e => e.id !== id && xyzTv({ x: e.position.x, y: position.y, z: e.position.z }).distanceTo(xyzTv(position)) < distance * Chunks.chunkSize);
    ...
    static thinkNoAttackTarget(entity) {
        ...
        if (entity.restTime.current > entity.restTime.currentMax) {
            ...
            if (!entity.init) {
                entity.init = true;
                if (Random.from(0, 3) === 2 && entity.reference.entity.ai.movement?.random)
                    Entities.selectRandomTarget(entity);
            }
    ...
    static update() {
        ...
        this.entities.forEach(entity => {
            ...
            if (this.updateTime > this.updateInterval + 10)
                Sockets.emit('entity:update', {
                    id: entity.id,
                    position: entity.position,
...
Entities.entities = [];
Entities.updateTime = 0;
Entities.updateInterval = 2;
